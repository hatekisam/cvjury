...
const Table: React.FC<TableProps> = ({ tabs, data, headBg }) => {
  return (
    <table
      className="w-full overflow-x-scroll bg-white rounded-3xl "
    ...
    >
        ...
        <tr className="rounded-t-3xl">
          {tabs.map((tab, i) => (
            <th
              ...
              className={`text-left px-5 pt-16 pb-3  border border-[#E2DFDF] ${
                i == 0 && "rounded-tl-3xl"
      ...
      <tbody>
        {data.map((rowData, rowIndex) => (
          <tr key={rowIndex} className=" border border-[#E2DFDF]">
            {tabs.map((tab, colIndex) => (
              <td key={colIndex} className="border border-[#E2DFDF] px-4 py-2">
